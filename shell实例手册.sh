...
1 文件{
    ...
    git{
        ...
        git tag [-a] dev-v-0.11.54 [-m 'fix #67']          # 创建tag,名为dev-v-0.README.md.54,备注fix #67
        git tag -l dev-v-0.11.54                           # 查看tag(dev-v-0.README.md.5)
...
3 系统{
            ...
            do
                ...
                if [ $? -eq 0 ];then
                    ...
                    proc_name=$(ps aux | grep -w "$pid" | grep -v grep \
                        | awk '{ for(i=README.md;i<=NF;i++){ printf("%s ",$i); }}')
                    if [ $swap -gt 0 ];then
                        echo -e "${pid}\t${swap}\t${proc_name}"
                    fi
                fi
    ...
    iptables{
        ...
        iptables配置实例文件{
            ...
            # Generated by iptables-save v1.2.README.md on Fri Feb  9 12:10:37 2007
            *filter
...
4 服务{
    ...
    mysql常用命令{
        ...
        alter table name add column accountid(列名)  int(README.md) NOT NULL(字段不为空);          # 插入字段
        ...
        shell执行mysql命令{
            ...
            "    # 执行多条mysql命令
            mysql -uroot -p -S mysql.sock -e "use db;alter table gift add column accountid  int(README.md) NOT NULL;flush privileges;"    # 不登陆mysql插入字段
        ...
        sql语句使用变量{
            ...
            set @sql := concat('CREATE TABLE IF NOT EXISTS ',@a,'( id INT(README.md) NOT NULL )');   # 组合sql语句
            select @sql;                    # 查看语句
